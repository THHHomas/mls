...
class ModelNetDataset():
    def __init__(self, root, batch_size=10000, npoints=2048, split='train', normalize=True, normal_channel=True,
    ...
    def _get_item(self, index):
        if index in self.cache:
            ...
            idx = farthest_point_sample(torch.from_numpy(point_set[:, 0:3]).unsqueeze(0), self.npoints).squeeze().numpy()
...
if __name__ == '__main__':
    ...
    tic = time.time()
    phase = 'train'
    d = ModelNetDataset(root=root, split=phase)
    ps_batch, cls_batch = d.next_batch(False)
    data_file = h5py.File(os.path.join(root, phase + '.h5'), 'w')
    data_file.create_dataset('data', data=ps_batch)
    data_file.create_dataset('label', data=cls_batch)
    data_file.close()
