...
from learning_based_surface.segnet import SurfaceNet, MainNet
...
def main(args):
    ...
    classifier = MainNet(50, normal=False, random_sp=False,
                         random_nb=False).cuda()
    ...
    auxiliarypath = auxiliarypath = os.path.join(root, 'auxiliary')
    construct_MLS_multi(TRAIN_DATASET.datapath, auxiliarypath, classifier.point_num)
    logger.info('construct_MLS for test data...')
    construct_MLS_multi(test_data, auxiliarypath, classifier.point_num, phase='test')

    train_neighbor_lists, train_data_idx_lists, train_local_axises = loadAuxiliaryInfo(auxiliarypath,
                                                                                       classifier.point_num)
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for i, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            loss = F.nll_loss(seg_pred, target.long())
            loss.backward()
            optimizer.step()
