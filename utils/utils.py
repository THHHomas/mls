...
def points2pcd(points, filename):
    # 存放路径
    PCD_DIR_PATH = os.path.join(os.path.abspath('.'), 'pcd')
    PCD_FILE_PATH = os.path.join(PCD_DIR_PATH, filename+'.pcd')
...
def construct_planes(points):
    """construct local planes for points"""
    return 0
...
def test(classifier, loader):
    ...
    for j, data in enumerate(loader, 0):
        if len(data)==4:
            ...
            # target = target[:, 0]
            points, target, neighbor_lists, data_idx_lists = points[:,:,0:3].float().cuda(), \
            target.cuda(), neighbor_lists.cuda(), data_idx_lists.cuda()
            # classifier = model.eval()
            with torch.no_grad():
                pred, _ = classifier(points, neighbor_lists, data_idx_lists)
        elif len(data)==2:
            points, target = data
